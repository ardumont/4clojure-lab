(ns ^{:doc "http://www.4clojure.com/problem/146#prob-title"}
  my-4clojure-lab.core-146
  (:use clojure.repl)
  (:use clojure.java.javadoc)
  (:use [midje.sweet])
  (:use [clojure.pprint :only [pprint]]))

(println "--------- BEGIN 146  ----------" (java.util.Date.))
...
;; for, because you can produce a sequence and feed it into a map, for example.
...
(fact 
  ...
  (trees-into-tables '{[1] {a b c d}
                       [2] {q r s t u v w x}}) => '{[[1] a] b, [[1] c] d,
                                                    [[2] q] r, [[2] s] t,
                                                    [[2] u] v, [[2] w] x}
  (trees-into-tables '{m {1 [a b c] 3 nil}}) => '{[m 1] [a b c], [m 3] nil})

(println "--------- END 146  ----------" (java.util.Date.))
